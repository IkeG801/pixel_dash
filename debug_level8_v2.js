// Debug Level 8 elevator issue - UPDATED with current code

function buildSkyLevels() {
  const skyNames = ['Cloudstep Start', 'Morning Lift', 'Featherline', 'Rose Horizon', 'Sunwake Rungs',
...
sortedPlats.forEach((p, idx) => {
  ...
  console.log(`  ${idx}: ${typeNames[p.type] || p.type} at y=${p.y.toFixed(0)} x=${p.x.toFixed(0)}-${(p.x+p.w).toFixed(0)}`);
});

console.log('\nPlatform reachability (Y-sorted):');
for (let i = 0; i < sortedPlats.length - 1; i++) {
  const curr = sortedPlats[i];
  const next = sortedPlats[i + 1];
  
  // Check horizontal overlap
  const overlapStart = Math.max(curr.x, next.x);
  const overlapEnd = Math.min(curr.x + curr.w, next.x + next.w);
  ...
  const vGap = next.y - (curr.y + curr.h);
  const hOverlap = overlapAmount > 0 ? overlapAmount : 0;
  
  console.log(`  ${currTypeNames[curr.type]}(${curr.y.toFixed(0)}) → ${nextTypeNames[next.type]}(${next.y.toFixed(0)}): vGap=${vGap.toFixed(0)}, hOverlap=${hOverlap.toFixed(0)}`);
  
  if (hOverlap < 20 && vGap > 0) {
    console.log(`    ⚠️  PROBLEM: Not enough horizontal overlap (${hOverlap.toFixed(0)}px)`);
  }
}
